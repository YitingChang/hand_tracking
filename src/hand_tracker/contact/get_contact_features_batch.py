...
def load_tracking_frame(session_name, trial_name, frame_number):
    """Loads 3D filter coordinates file and slices the precise frame index row."""
    pose_3d_dir = ANALYSIS_ROOT / session_name / 'anipose' / 'pose_3d_filter'
    csv_path = pose_3d_dir / f'{trial_name}_f3d.csv'
    df = pd.read_csv(csv_path)
    return df.iloc[frame_number]
...
def process_single_trial_pipeline(session_name, trial_name, log_fname, frame_number):
    ...
    frame_row = load_tracking_frame(session_name, trial_name, frame_number)
    
    # Load Object and setup spatial matrix alignment
