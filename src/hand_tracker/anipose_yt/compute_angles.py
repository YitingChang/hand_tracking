...
# edited by Yiting on 2024/12/05
def process_trial(config, session_name, trial_name):
    ...
    if config['filter3d']['enabled']:
        pipeline_3d = config['pipeline']['pose_3d_filter']
        pose3d_fname = os.path.join(pipeline_analysis_dir, session_name,
    ...
    outdir = os.path.join(pipeline_analysis_dir, session_name,
                          'anipose', pipeline_angles)

    if len(pose3d_fname) > 0:
        os.makedirs(outdir, exist_ok=True)
    
    out_fname = os.path.join(outdir, trial_name + '_angles.csv')
    ...
    compute_angles(config, pose3d_fname, out_fname)
