...
CONTACT_RDM_DIR = ANALYSIS_ROOT / "contact_analysis" / "contact_rdms"
CONTACT_TSNE_DIR = ANALYSIS_ROOT / "contact_analysis" / "contact_tsne"
os.makedirs(CONTACT_TSNE_DIR, exist_ok=True)
...
def compute_tsne_pca(df, feature_names, tsne_perplexity=30):
    # Ensure a copy to avoid SettingWithCopy warnings
    df = df.copy()
    ...
    if feature_names[0] == 'contact_vector':
        # Check if the data is already a NumPy array or list.
        # If it's a string, use literal_eval. If it's already an array, use it directly.
        def safe_convert(x):
            if isinstance(x, str):
                return literal_eval(x)
            return x

        # Convert to a list/array and then stack
        df_features = np.vstack(df['contact_vector'].apply(safe_convert).values)
    else:
        # Prepare features for t-SNE using explicit column names (such as for hand features)
        df_features = df[feature_names].values
...
def plot_dim_red(df, method="tsne", scatter_color_mode="monochrome", overlay=False, zoom=0.25):
    ...
    base_name = f"contact_{TRIAL_TYPE}_{FRAME_NUMBER}_{method}_{scatter_color_mode}"
    ...
    save_path = os.path.join(CONTACT_TSNE_DIR, save_fname)
...
def get_feature_contribution(X_scaled, tsne_features, feature_names):
    ...
    save_path = os.path.join(CONTACT_TSNE_DIR, f"feature_importance_{TRIAL_TYPE}_f{FRAME_NUMBER}_tsne_perplexity{TSNE_PERPLEXITY}.png")
    ...
    plt.savefig(os.path.join(CONTACT_TSNE_DIR, f"feature_importance_d1_{TRIAL_TYPE}_f{FRAME_NUMBER}_tsne_perplexity{TSNE_PERPLEXITY}.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if shap_d2 is not None:
        ...
        plt.savefig(os.path.join(CONTACT_TSNE_DIR, f"feature_importance_d2_{TRIAL_TYPE}_f{FRAME_NUMBER}_tsne_perplexity{TSNE_PERPLEXITY}.png"), dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    # --- 1. Load Data ---
    contact_feat_pkl = f"contact_avg_features_{TRIAL_TYPE}_{ori_str}.pkl"
    contact_feat_path = CONTACT_RDM_DIR / contact_feat_pkl
    df_contact = pd.read_pickle(contact_feat_path)
    feature_names = list(df_contact.keys()[1:]) # the first column is shape_id

    # Compute tSNE (Pass feature_names explicitly)
    df_tsne_pca, X_scaled, tsne_features, principal_components = compute_tsne_pca(df_contact, feature_names, tsne_perplexity=TSNE_PERPLEXITY)
    ...
    # Save tSNE results
    df_tsne_pca.to_csv(os.path.join(CONTACT_TSNE_DIR, f"contact_conf_{TRIAL_TYPE}_f{FRAME_NUMBER}_pca_tsne_perplexity{TSNE_PERPLEXITY}.csv"), index=False)
