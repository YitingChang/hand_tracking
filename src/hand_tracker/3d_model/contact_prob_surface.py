...
HAND_OPACITY = 0.5 
OBJECT_OPACITY = 0.5 
DISTANCE_THRESHOLD_MM = -10  # Threshold for considering a point in contact with the object
...
# --- 3. Surface Contact Probability ---
def get_surface_contact_prob(mesh, finger_chains, palm_pts, normal, diameter=8.0, palm_thick=9.0):
    # 1. Setup Object Surface KDTree for fast lookups
    obj_tree = KDTree(mesh.vertices)
    
    surface_points = []
    
    # 2. Sample Finger Surfaces (Bottom Half)
    for name, chain in finger_chains.items():
        for i in range(len(chain)-1):
            p1 = get_xyz(chain[i])
            p2 = get_xyz(chain[i+1])
            
            # Create a simple "cylinder surface" sampling
            # We sample points around the circumference but only keep the 'bottom' 180 degrees
            v_seg = p2 - p1
            dist_seg = np.linalg.norm(v_seg)
            num_steps = int(dist_seg / 2) # Sample every 2mm along length
            
            for s in np.linspace(0, 1, num_steps):
                center = p1 + s * v_seg
                # Generate a small ring of points (simplified)
                # Keep only those where the vector from center points 'down' (along -normal)
                for angle in np.linspace(0, 2*np.pi, 8):
                    # Point on the radius of the finger
                    # (In a real model, we'd use a local coordinate frame, 
                    # but checking against the palm normal is a good heuristic)
                    test_pt = center + (np.array([np.cos(angle), np.sin(angle), 0]) * (diameter/2))
                    if np.dot(test_pt - center, -normal) > 0: # Facing 'down'
                        surface_points.append(test_pt)

    # 3. Sample Palm Surface (Bottom Face)
    # Use the bottom_pts we calculated for the hull
    palm_surface_pts = palm_pts - (normal * (palm_thick / 2.0))
    surface_points.extend(palm_surface_pts)

    surface_points = np.array(surface_points)
    
    # 4. Calculate Distances to Object
    # We define 'contact' as being within 1mm of the surface or inside it
    distances, _ = obj_tree.query(surface_points)
    
    # Probability = (Points touching or inside) / (Total facing-surface points)
    contact_count = np.sum(distances < DISTANCE_THRESHOLD_MM) 
    probability = contact_count / len(surface_points)
    
    # For visualization, we can return the points that are considered in contact
    contact_surface_points = surface_points[distances < DISTANCE_THRESHOLD_MM]

    return probability, surface_points, contact_surface_points
...
# E. Plot Contact Points (Yellow Spheres) 
prob, surface_cloud, contact_cloud = get_surface_contact_prob(mesh, finger_chains, palm_pts, normal)
ax.scatter(contact_cloud[:,0], contact_cloud[:,1], contact_cloud[:,2], 
           color='g', s=10, alpha=0.9, label='Contact Surface Area')

# --- 4. FORMATTING ---
ax.set_title("3D Hand Reconstruction: Contact Points")
