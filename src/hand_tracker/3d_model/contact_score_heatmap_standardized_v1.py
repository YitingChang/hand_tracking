...
trial_name = "2025-12-09_14-18-17"
...
log_dir = RAW_DATA_ROOT / session_name / 'trial_logs'
log_fnames = find_matching_log([csv_file], log_dir)
if log_fnames:
    with open(log_fnames[0], 'r') as file:
...
if has_halted_motion:
    ...
    recon_dir = ANALYSIS_ROOT / session_name / 'reconstructions' / trial_name
    recon_dir.mkdir(parents=True, exist_ok=True)
    img_path = recon_dir / f'contact_scores_standardized_heatmap_{trial_name}_f{FRAME_NUMBER}_v1.png'
    plt.savefig(img_path, dpi=300)
else:
    print("Hand does not grabbed the object. Skipping reconstruction and heatmap generation.")