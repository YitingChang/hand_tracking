...
def visualize_combined(config, pose_fname, cgroup, offsets_dict,
                       fnames_2d, fname_3d, out_fname, angle_fname):
...
# edited by Yiting on 2024/12/02
def process_trial(config, session_name, trial_name):
    ...
    pipeline_videos_combined = config['pipeline']['videos_combined']
    angle_fname = os.path.join(pipeline_analysis_dir, session_name,
                        "anipose", config['pipeline']['angles'], trial_name + '_angles.csv')
    if config['filter3d']['enabled']:
        pipeline_pose_3d = config['pipeline']['pose_3d_filter']
        pipeline_videos_labeled_3d = config['pipeline']['videos_labeled_3d_filter']
        vid_fname_3d = os.path.join(pipeline_analysis_dir, session_name,
                                "anipose", pipeline_videos_labeled_3d, trial_name + "_f3d.mp4")
        pose_fname = os.path.join(pipeline_analysis_dir, session_name,
                        "anipose", pipeline_pose_3d, trial_name + '_f3d.csv')
    else:
        pipeline_pose_3d = config['pipeline']['pose_3d']
        pipeline_videos_labeled_3d = config['pipeline']['videos_labeled_3d']
        vid_fname_3d = os.path.join(pipeline_analysis_dir, session_name,
                                "anipose", pipeline_videos_labeled_3d, trial_name + "_3d.mp4")
        pose_fname = os.path.join(pipeline_analysis_dir, session_name,
                        "anipose", pipeline_pose_3d, trial_name + '_3d.csv')

    video_ext = config['video_extension']

    vid_fnames_2d = glob(os.path.join(pipeline_data_dir, session_name,
                                       "cameras", trial_name, "*."+video_ext))
    vid_fnames_2d = sorted(vid_fnames_2d, key=natural_keys)

    cgroup = None
    calib_folder = os.path.join(pipeline_analysis_dir, pipeline_calibration)
    ...
    outdir = os.path.join(pipeline_analysis_dir, session_name,
                                      "anipose", pipeline_videos_combined)

    if len(vid_fname_3d) > 0:
        os.makedirs(outdir, exist_ok=True)

    out_fname = os.path.join(outdir, trial_name + '_2d3d.mp4')

    if os.path.exists(out_fname) and \
        abs(get_nframes(out_fname) - get_nframes(vid_fname_3d)) < 100:
        return
    ...
    cam_names = [get_cam_name(config, fname) for fname in vid_fnames_2d]
    ...
    visualize_combined(config, pose_fname, cgroup_subset, offsets_dict,
                        vid_fnames_2d, vid_fname_3d, out_fname, angle_fname)
