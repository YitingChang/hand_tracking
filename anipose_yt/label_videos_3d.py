...
def process_trial(config, session_name, trial_name, filtered=False):
    ...
    if filtered:
        pipeline_videos_labeled_3d = config['pipeline']['videos_labeled_3d_filter']
        pipeline_3d = config['pipeline']['pose_3d_filter']
        labels_fname = os.path.join(pipeline_analysis_dir, session_name,
                                "anipose", pipeline_3d, trial_name + '_f3d.csv')
        outdir = os.path.join(pipeline_analysis_dir, session_name,
                                      "anipose", pipeline_videos_labeled_3d)
        out_fname = os.path.join(outdir, trial_name + '_f3d.mp4')
    else:
        pipeline_videos_labeled_3d = config['pipeline']['videos_labeled_3d']
        pipeline_3d = config['pipeline']['pose_3d']
        labels_fname = os.path.join(pipeline_analysis_dir, session_name,
                        "anipose", pipeline_3d, trial_name + '_3d.csv')
        outdir = os.path.join(pipeline_analysis_dir, session_name,
                                      "anipose", pipeline_videos_labeled_3d)
        out_fname = os.path.join(outdir, trial_name + '_3d.mp4')
    ...
    some_vid = vid_fnames[0]
    params = get_video_params(some_vid)

    visualize_labels(config, labels_fname, out_fname, params['fps'])
